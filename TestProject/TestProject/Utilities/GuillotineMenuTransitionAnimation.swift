...
protocol GuillotineMenu: NSObjectProtocol {
    ...
    optional var titleLabel: UIButton! { get }
    optional var sideLabel: UIButton! { get }
...
class GuillotineTransitionAnimation: NSObject {
    ...
    private func animateMenu(view: UIView, context:UIViewControllerContextTransitioning) {
        ...
        if self.mode == .Presentation {
            if supportView != nil {
                ...
                showHostSideLabel(false,animated:true)
            }
            ...
            if supportView != nil {
                ...
                showHostSideLabel(true,animated:true)
    ...
    private func showHostSideLabel(show: Bool, animated: Bool) {
        if let guillotineMenu = menu as? GuillotineMenu {
            let buttonWidth = guillotineMenu.sideLabel!.frame.size.height
            guard guillotineMenu.sideLabel != nil else { return }
            guillotineMenu.sideLabel!.center = CGPointMake(CGRectGetHeight(supportView!.frame)/2, CGRectGetWidth(supportView!.frame) - (buttonWidth+6))
            guillotineMenu.sideLabel!.transform = CGAffineTransformMakeRotation(degreesToRadians(90));
            menu.view.addSubview(guillotineMenu.sideLabel!)
            if mode == .Presentation {
                guillotineMenu.sideLabel!.alpha = 1;
            } else {
                guillotineMenu.sideLabel!.alpha = 0;
            }
            
            if animated {
                UIView.animateWithDuration(duration, animations: {
                    guillotineMenu.sideLabel!.alpha = CGFloat(show)
                    }, completion: nil)
            } else {
                guillotineMenu.sideLabel!.alpha = CGFloat(show)
            }
